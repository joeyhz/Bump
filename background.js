...
chrome.storage.sync.get('userid', function (items) {
    ...
    firebase.database().ref('Connections/' + id + '/friend').on('value', function (friendID) {
        console.log('wave from '+ friendID.val());
        if (friendID.val()) {
    ...
    firebase.database().ref('Activity/' + uid).once('value', function (snap) {
        ...
        if (myP && myT) {
            frList.forEach(function (friend) {
                firebase.database().ref('Activity/' + friend).off()
                firebase.database().ref('Activity/' + friend).on('value',
                    function (snapshot) {
                        console.log('Friend entering:', snapshot.child('title').val(), myT)
...
function match(fid, info, type) {
    //Found a suitable match, giving option to reach out
    var accept;
